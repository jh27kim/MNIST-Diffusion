...
def train_mnist():
    ...
    now = datetime.now()
    save_dir = f'./output/canonical_concat_{now.strftime("%Y%m%d_%H%M%S")}/'
    ...
    ddpm = CanonicalDDPM(nn_model=CanonicalContextUnet(in_channels=1, n_feat=n_feat, n_classes=n_classes), betas=(1e-4, 0.02), n_T=n_T, device=device, drop_prob=0.1)
    ddpm.to(device)
